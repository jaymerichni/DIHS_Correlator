...
Prepares the Italian benchmark dataset from the raw source tables expected under `data/raw/`, performs the main cleaning and harmonization steps, and writes the processed coupled dataset used by the downstream case-study analyses. This notebook supports the data-preparation stage behind the Italian benchmark case study in the manuscript.
...
Reproduces the Italian case study centered on the Caio outcrop and its relationship to the Roman Magmatic Province and related volcanic provinces. This notebook supports the main manuscript section devoted to empirical tephra-source attribution using a locally generated Italian benchmark dataset.
...
- Synthetic scenario data are bundled in the repository, but the Petrelli/GEOROC-derived Italian benchmark inputs and the derived `full_italian_data.csv` are not redistributed.
- Follow `data/raw/README.md` to obtain the benchmark source files and `data/processed/caio_italy_benchmark/README.md` to generate the local processed benchmark CSV required by `2_caio_source_attribution.ipynb` and `3_sensitivity_data_size.py`.
- When using `0_raw_data_preprocessing.ipynb` or the benchmark materials it reconstructs, please cite both this DIHS manuscript and the original Petrelli et al. (2017) source described in `data/raw/README.md`.
